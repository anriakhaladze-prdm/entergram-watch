...
import { parseFilters, matchRow, filterLabel, queueHref, SETS } from '../components/filters';
...
export default function Queue() {
  ...
  const active = fl.f.length ? fl.f : (router.isReady && Object.keys(router.query).length ? [] : DEFAULT_F);
  ...
  const toggleF = (k) => {
    const cur = new Set(active.filter((x) => x !== 'actionable' || k === 'actionable'));
    if (k === 'actionable') { setParams({ f: cur.has('actionable') ? ['hosted'] : ['actionable'] }); return; }
    cur.has(k) ? cur.delete(k) : cur.add(k);
    setParams({ f: [...cur].length ? [...cur] : ['hosted'] });
  };

  const hosts = useMemo(() => {
    const m = new Map();
    for (const r of rows) { if (r.flags.left || r.flags.unhosted) continue; const k = r.host || 'unattributed'; m.set(k, (m.get(k) || 0) + 1); }
    return [...m.entries()].sort((a, b) => b[1] - a[1]);
  }, [rows]);
  const counts = useMemo(() => {
    const c = {};
    for (const r of rows) c[r.state] = (c[r.state] || 0) + 1;
    c.actionable = rows.filter((r) => r.actionable).length;
    return c;
  }, [rows]);

  const shown = useMemo(() => {
    const filters = { ...fl, f: active, q };
  ...
  }, [rows, fl, active, q, sort]);

  const open = fl.chat ? rows.find((r) => String(r.chatId) === String(fl.chat)) : null;
  ...
  const toolbar = (
    <>
      ...
      <span className={`th-chip typ-label-small${active.includes('actionable') ? ' is-on' : ''}`} onClick={() => toggleF('actionable')} role="button" tabIndex={0} aria-pressed={active.includes('actionable')} data-tip="waiting, unhappy, no contact, not responding">
        Needs action {counts.actionable ?? 0}
      </span>
      {STATES.map((st) => (
        <span key={st.key} className={`th-chip typ-label-small${active.includes(st.key) ? ' is-on' : ''}`} onClick={() => toggleF(st.key)} role="button" tabIndex={0} aria-pressed={active.includes(st.key)} data-tip={st.blurb}>
      ...
      {error || !snap ? <Empty error={error} /> : (
        <>
          <div className="ow-hostbar">
            ...
            {(fl.mood || fl.silence || fl.reply || active.some((k) => SETS[k] && k !== 'actionable' && k !== 'hosted')) ? (
              <span className="th-chip typ-label-small ow-chip-filter is-on" onClick={() => setParams({ mood: null, silence: null, reply: null, f: active.includes('hosted') ? ['hosted'] : ['actionable'] })} role="button" tabIndex={0} title="Clear this filter">
                {filterLabel({ ...fl, f: active.filter((k) => SETS[k] && k !== 'actionable' && k !== 'hosted') })} <Icon name="close" size={10} />
              </span>
            ) : null}
          </div>
