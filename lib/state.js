...
const ms = (d) => (d == null ? 0 : typeof d === 'number' ? d : new Date(d).getTime());
...
const K = {
  ...
  tally: 'ew2:tally',
  signins: 'ew2:signins',
  sessions: 'ew2:sessions',
  revoked: (sid) => `ew2:revoked:${sid}`,
  blocked: 'ew2:blocked',
  allowed: 'ew2:allowed',
  excluded: 'ew2:excluded',
};
...
// What the full sweep left out. Its own key rather than part of the metadata,
// which is rewritten on every tick; this changes only on a full sweep.
export const putExcluded = (rows) => one(['SET', K.excluded, JSON.stringify(rows), 'EX', String(7 * 86400)]);
export const getExcluded = async () => parse(await one(['GET', K.excluded])) || [];

/* ---- access: sign-ins, sessions, the block-list ------------------------------
   Sessions are JWTs, so revocation needs a registry: every sign-in is recorded
   under its session id, a revoke marks the id for as long as a token can live,
   and every session read checks the mark (through a short cache, lib/gate.js). */
const SESSION_TTL_SEC = 9 * 3600;
const hashToList = (res) => {
  if (!res) return [];
  const out = [];
  if (Array.isArray(res)) { for (let i = 0; i < res.length; i += 2) { const v = parse(res[i + 1]); if (v) out.push(v); } }
  else if (typeof res === 'object') { for (const v of Object.values(res)) { const p = parse(v); if (p) out.push(p); } }
...
export const logSignIn = (entry) => pipeline([['LPUSH', K.signins, JSON.stringify(entry)], ['LTRIM', K.signins, '0', '499']]);
export async function getSignIns(limit = 200) {
  const res = await one(['LRANGE', K.signins, '0', String(limit - 1)]);
  return (res || []).map(parse).filter(Boolean);
}

export const putSession = (rec) => one(['HSET', K.sessions, rec.sid, JSON.stringify(rec)]);
export async function listSessions({ now = Date.now(), maxAgeMs = 8 * 3600000 } = {}) {
  const rows = hashToList(await one(['HGETALL', K.sessions]));
  // A token past its own lifetime is no session; drop the record with it.
  const stale = rows.filter((r) => now - ms(r.createdAt) > maxAgeMs);
  if (stale.length) await pipeline(stale.map((r) => ['HDEL', K.sessions, r.sid]));
  return rows.filter((r) => now - ms(r.createdAt) <= maxAgeMs).sort((a, b) => ms(b.lastSeen || b.createdAt) - ms(a.lastSeen || a.createdAt));
}
export async function touchSession(sid, at = Date.now()) {
  const cur = parse(await one(['HGET', K.sessions, sid]));
  if (!cur) return;
  await one(['HSET', K.sessions, sid, JSON.stringify({ ...cur, lastSeen: at })]);
}
export const revokeSession = (sid, by = '') => pipeline([['SET', K.revoked(sid), JSON.stringify({ by, at: Date.now() }), 'EX', String(SESSION_TTL_SEC)], ['HDEL', K.sessions, sid]]);
export const isSessionRevoked = async (sid) => Boolean(await one(['GET', K.revoked(sid)]));
export async function revokeSessionsOf(email, by = '') {
  const rows = hashToList(await one(['HGETALL', K.sessions])).filter((r) => String(r.email || '').toLowerCase() === String(email).toLowerCase());
  for (const r of rows) await revokeSession(r.sid, by);
  return rows.length;
}

export async function listBlocked() {
  return hashToList(await one(['HGETALL', K.blocked])).sort((a, b) => ms(b.at) - ms(a.at));
}
export const blockEmail = (email, by = '') => one(['HSET', K.blocked, String(email).toLowerCase(), JSON.stringify({ email: String(email).toLowerCase(), by, at: Date.now() })]);
export const unblockEmail = (email) => one(['HDEL', K.blocked, String(email).toLowerCase()]);
export const isEmailBlocked = async (email) => Boolean(await one(['HGET', K.blocked, String(email || '').toLowerCase()]));

export async function listAllowed() {
  return hashToList(await one(['HGETALL', K.allowed])).sort((a, b) => ms(b.at) - ms(a.at));
}
export const allowEmail = (email, by = '') => one(['HSET', K.allowed, String(email).toLowerCase(), JSON.stringify({ email: String(email).toLowerCase(), by, at: Date.now() })]);
export const disallowEmail = (email) => one(['HDEL', K.allowed, String(email).toLowerCase()]);
export const isEmailAllowed = async (email) => Boolean(await one(['HGET', K.allowed, String(email || '').toLowerCase()]));
