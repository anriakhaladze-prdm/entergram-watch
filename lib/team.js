// Who is staff. Nobody owns a chat, so this table only separates our side of
// a conversation from the player's.
//
...
// Neither side: a sender who sits in many player groups without being staff
// or the player (an outside VIP agent, say). Listed so the learned-staff rule
// below cannot claim them, and so their messages count for nobody.
const THIRD = new Map((team.thirdParties || []).map((p) => [String(p.telegramUserId), p.name || `sender ${p.telegramUserId}`]));
export const isThirdParty = (id) => id != null && THIRD.has(String(id));
export const thirdPartyName = (id) => (id != null ? THIRD.get(String(id)) || null : null);
...
export function isStaffSender(id, name) {
  ...
  if (isThirdParty(id)) return false;
...
export const isPlayerSender = (id, name) => id != null && !isBot(id) && !isThirdParty(id) && !isStaffSender(id, name);
