...
export function deriveRow(meta, state, { now = Date.now(), quietDays = 7, unansweredHours = 12 } = {}) {
  ...
  // The group was opened, we posted, and the player has never been in it. Only
  // provable on a history we can see the whole of, which the creation message
  // marks; without it, silence could just be messages from before we could
  // read. A player who joined and never spoke is not this: they are here.
  const notJoined = historyRead && fromText && Boolean(f.createdAt) && f.playerSeen === false && !f.playerJoinedAt;

  const flags = {
    left,
    not_joined: notJoined && !left,
  ...
  else if (flags.not_joined) { stateKey = 'not_joined'; signals.push(`group opened ${humanAge(daysSince(f.createdAt, now))} ago, the player never arrived`); }
  ...
  return {
    ...
    accountStatus: state?.custom?.accountStatus || null,
    selfExcludedUntil: state?.custom?.selfExcludedUntil || null,
    favouriteGames: state?.custom?.favouriteGames || null,
    favouriteProviders: state?.custom?.favouriteProviders || [],
    typicalBetUsd: state?.custom?.typicalBetUsd ?? null,
    signupDate: state?.custom?.signupDate || null,
    sportsbook: Boolean(state?.custom?.sportsbook),
    originals: Boolean(state?.custom?.originals),
    slots: Boolean(state?.custom?.slots),
    liveCasino: Boolean(state?.custom?.liveCasino),
...
// Aggregates for the overview. active: a real conversation with a player who
// is in the group, the set every working figure is over.
export function summarize(rows) {
  ...
  return {
    ...
    notJoined: rows.filter((r) => r.flags.not_joined).length,
