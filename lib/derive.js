...
export function deriveRow(meta, state, { now = Date.now(), quietDays = 7, unansweredHours = 12 } = {}) {
  // Facts from a history read when there is one, otherwise from the event
  // stream, which knows who spoke when but not what was said.
...
export function summarize(rows) {
  // hosted: a player with a host. active: hosted and not time-barred, which is
  // the set every working figure is computed over.
