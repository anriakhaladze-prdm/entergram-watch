...
import { isPlayerChat, dedupeChats, chatMeta } from './scope.js';
import { extractFacts, normalizeMessage } from './facts.js';
import { deriveRow, summarize } from './derive.js';
import { READER_ACCOUNT_ID, useLearnedStaff, identify, isSharedSender, isBot, looksStaffName } from './team.js';
...
export function updateTally(tally, chatId, messages) {
  ...
  for (const m of messages) {
    const id = m.senderId;
    if (!id || m.actionType || isBot(id) || isSharedSender(id) || identify(id, m.senderName)) continue;
...
export async function runScan({
  ...
  try {
    ...
    if (full) {
      ...
      bookChats = deduped.filter(isPlayerChat).map((c) => {
        const m = chatMeta(c);
    ...
    for (const { c, st } of toRead) {
      ...
      const base = { ...(st || {}), lastMessageAt: c.lastMessageAt };
      try {
        ...
        if (r.items.length) {
          ...
          const { facts, playerTexts, playerQuote } = extractFacts(msgs, { player: c.player, now });
          Object.assign(base, { facts, historyAt: clock(), historyUnavailable: false });
          texts.set(c.chatId, { playerTexts, quote: playerQuote, facts });
          read++;
        } else {
    ...
    // --- sentiment ----------------------------------------------------------
    // Rules on every chat with fresh player text; the model only on chats whose
    // player has said something new since they were last scored.
    ...
    for (const [chatId, t] of texts) {
      ...
      if (stored && stored.lastPlayerAt === t.facts.lastPlayerAt) { st.sentiment = stored; continue; }
      const rules = ruleScan(t.playerTexts);
      st.sentiment = { ...rules, quote: t.quote, lastPlayerAt: t.facts.lastPlayerAt, scoredAt: new Date(now).toISOString() };
      modelCandidates.push({ id: chatId, texts: t.playerTexts, rules, st, quote: t.quote, lastPlayerAt: t.facts.lastPlayerAt });
    }
    if (provider && modelCandidates.length && elapsed() < softDeadlineMs) {
      const budgetS = Number(process.env.SENTIMENT_BUDGET || 40);
      const ordered = modelCandidates.sort((a, b) => (NEGATIVE.has(b.rules.label) - NEGATIVE.has(a.rules.label))).slice(0, budgetS);
      const scored = await scoreWithModel(ordered.map(({ id, texts: tx }) => ({ id, texts: tx })));
      for (const c of ordered) {
        const m = scored.get(String(c.id));
        if (m) c.st.sentiment = { ...mergeSentiment(c.rules, m), quote: c.quote, lastPlayerAt: c.lastPlayerAt, scoredAt: new Date(now).toISOString() };
      }
      log(`sentiment: ${provider}, ${ordered.length} scored by the model of ${modelCandidates.length} changed`);
    } else if (modelCandidates.length) {
      log(`sentiment: rules only for ${modelCandidates.length} changed chats${provider ? '' : ' (no model key configured)'}`);
    }
    ...
    if (!seeded) {
      ...
      let seededN = 0;
      for (const r of rows) {
        if (!r.hostActive || r.flags.left) continue;
      ...
      for (const d of batch) {
        ...
        if (!dryRun) {
          try {
            ...
            alertLog.push({ at: clock(), kind: d.kind, chatId: d.row.chatId, player: d.row.player, host: d.row.host, ts: r.ts || null, text });
            posted++;
          ...
          posted++;
          alertLog.push({ at: clock(), kind: d.kind, chatId: d.row.chatId, player: d.row.player, host: d.row.host, dryRun: true, text });
        }
      }
    ...
    if (persist) {
      ...
      await kv.putMeta({ ...meta, lastTickAt: iso, lastFullAt: full ? iso : meta.lastFullAt, seededAt: meta.seededAt || (!seeded ? iso : null), lastGoodAt: iso });
    }
