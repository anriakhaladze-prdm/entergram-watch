...
export async function runScan({
  ...
  try {
    ...
    if (full) {
      ...
      // Membership when the sweep answered, the account on the row otherwise,
      // so a failed call narrows nothing. Private chats are never in the
      // groups endpoint, so the account check always carries those.
      if (groups.ok) scopeIds = groups.ids;
      ...
      log(`full sweep: ${items.length} entries, ${sweptChats} distinct chats, ${scopeIds.size} in the shared account, ${bookChats.length} player chats, ${excluded.length} left out, ${links.size} invite links`);
    } else {
