...
export async function runScan({
  ...
  try {
    ...
    // Refresh profile attributes independently from message-history rotation.
    // Bumping the version forces one complete refresh after this deployment.
    let cfReads = 0, deadlineHit = false;
    const customDue = bookChats.filter((c) => {
      const st = next.get(c.chatId) || prior.get(c.chatId);
      return st?.customVersion !== CUSTOM_VERSION || !st?.customAt || now - ms(st.customAt) > DAY;
    });
    for (let i = 0; i < customDue.length; i += 8) {
      if (elapsed() > softDeadlineMs) { deadlineHit = true; break; }
      const batch = customDue.slice(i, i + 8);
      await Promise.all(batch.map(async (c) => {
        const base = { ...(prior.get(c.chatId) || {}), ...(next.get(c.chatId) || {}), lastMessageAt: c.lastMessageAt };
        try {
          const cf = await api.chatCustomFields(c.chatId);
          const v = cf?.data?.values || cf?.values || {};
          base.custom = customFields(v);
          base.customAt = clock();
          base.customVersion = CUSTOM_VERSION;
          cfReads++;
        } catch { /* decoration only; retry next scan */ }
    ...
    useLearnedStaff(learnedStaff(tally));
    log(`history: ${read} read, ${unavailable} unavailable, ${toRead.length} attempted of ${candidates.length} due (${candidates.filter((x) => x.moved).length} moved); custom fields: ${cfReads}/${customDue.length} refreshed${deadlineHit ? ', stopped at the time budget' : ''}`);
