...
import { isKnownStaff, updateTally, learnedStaff, learnedNames, useLearnedStaff } from './staff.js';
import { getChatStates, putChatStates, isSeeded, markSeeded, putSnapshot, putRunLog, putDaily, getTally, putTally, kvConfigured } from './state.js';
...
// Sentiment reads the PLAYER only. A host writing "unfortunately there is no
// lossback at the moment" is our tone, not theirs, and scoring it would mean
// grading ourselves. This uses the learned registry rather than the static
// list, so a host who is not in config/staff.json still does not leak into the
// player's side of the transcript.
const looksStaffMsg = (m) => isKnownStaff(m.senderId, m.senderName) || m.isOut === true;
...
export async function runScan({
  ...
  // The learned staff registry, carried between runs.
  let tally = {};
  if (kvConfigured()) { try { tally = (await getTally()) || {}; } catch { tally = {}; } }
  useLearnedStaff(learnedStaff(tally), learnedNames(tally));
  ...
  // Everything read this run feeds the registry before anything is classified,
  // so a host discovered in one chat is recognised in the rest of the same run.
  for (const [chatId, msgs] of history) updateTally(tally, chatId, msgs);
  const staffIds = learnedStaff(tally);
  useLearnedStaff(staffIds, learnedNames(tally));
  log(`history: ${read} read, ${denied} unavailable, ${needsHistory.length} attempted of ${hosted.length} hosted`);
  log(`staff registry: ${staffIds.size} known staff accounts from ${Object.keys(tally).length} senders seen`);
  ...
  for (const r of rows) {
    ...
    // Newest first, so a long history does not drown the recent mood.
    const playerMsgs = msgs
      .filter((m) => !m.actionType && m.text && !looksStaffMsg(m))
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    const texts = playerMsgs.map((m) => m.text);
    ...
    // One short quote from the player, so a host can see what the verdict was
    // reacting to instead of taking a label on faith.
    const quote = (playerMsgs.find((m) => m.text && m.text.length > 12)?.text || playerMsgs[0].text || '').slice(0, 120);
    r.sentiment = { ...rules, quote, lastPlayerAt: r.lastPlayerAt, scoredAt: new Date(now).toISOString() };
    candidates.push({ id: r.chatId, texts, quote, row: r, rules });
  }
  ...
  if (provider && candidates.length) {
    ...
    for (const c of ordered) {
      ...
      if (m) c.row.sentiment = { ...mergeSentiment(c.rules, m), quote: c.quote, lastPlayerAt: c.row.lastPlayerAt, scoredAt: new Date(now).toISOString() };
    }
