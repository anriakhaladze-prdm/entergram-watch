...
import { isSessionRevoked, isEmailBlocked, isEmailAllowed, touchSession, kvConfigured } from './state.js';
import { isStaticallyBlocked, isStaticallyAllowed } from './access.js';
...
const allowed = new Map();   // email -> { v, at }
...
// On the list, one way or another. Without a store only the static lists
// count, so the whitelist still gets in.
export async function emailAllowed(email) {
  if (!email) return false;
  if (isStaticallyAllowed(email)) return true;
  if (!kvConfigured()) return false;
  return cached(allowed, String(email).toLowerCase(), () => isEmailAllowed(email));
}
export const hasAccess = async (email) => Boolean(email) && !(await emailBlocked(email)) && (await emailAllowed(email));
export function forget(sid, email) {
  if (sid) { revoked.delete(sid); touched.delete(sid); }
  if (email) { blocked.delete(String(email).toLowerCase()); allowed.delete(String(email).toLowerCase()); }
}
...
export const __reset = () => { revoked.clear(); blocked.clear(); allowed.clear(); touched.clear(); };
