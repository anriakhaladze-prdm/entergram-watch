...
// Which staff member hosts this player: the hosting team member who does the
// talking, message count first, most recent speech breaking a tie. Speech in
// the last 30 days is weighted so a book that has been handed over follows the
// new host rather than the old one's backlog.
export function attributeHost(speakers, now) {
  const recent = (s) => (now - ms(s.last) <= 30 * 86400000 ? 1 : 0);
  const total = speakers.reduce((a, s) => a + s.msgs, 0) || 1;
  const hosting = speakers.filter((s) => s.person?.hosting);
  hosting.sort((a, b) => (recent(b) - recent(a)) || (b.msgs - a.msgs) || (ms(b.last) - ms(a.last)));
  // These groups are worked as a pool: several hosts answer in the same chat.
  // The share says how much of our side of the conversation the named host
  // actually carried, so a 35% "host" is not mistaken for an owner.
  if (hosting[0]) return { name: hosting[0].person.name, email: hosting[0].person.email, telegramId: hosting[0].id, hosting: true, source: 'conversation', share: Math.round((hosting[0].msgs / total) * 100), others: hosting.length - 1 };
  const retired = speakers.find((s) => s.person && !s.person.hosting);
  const shared = speakers.find((s) => s.shared);
  if (shared && !retired) return { name: shared.name, email: null, telegramId: shared.id, hosting: true, source: 'shared account', unattributed: true };
  if (retired) return { name: retired.person.name, email: retired.person.email, telegramId: retired.id, hosting: false, source: 'conversation', reason: retired.person.reason };
  const unknown = speakers[0];
  if (unknown) return { name: unknown.name || `sender ${unknown.id}`, email: null, telegramId: unknown.id, hosting: true, source: 'conversation, unidentified', unidentified: true };
  return null;
}
...
export function extractFacts(messages, { player = null, now = Date.now() } = {}) {
  ...
  const playerMsgs = ordered.filter((m) => isPlayer(m) && m.text);
  ...
  const facts = {
    ...
    host: attributeHost(ranked, now),
    staffSpeakers: ranked.slice(0, 6).map((s) => ({ id: s.id, name: s.name, msgs: s.msgs, last: s.last, hosting: s.person ? s.person.hosting : !s.shared ? null : false })),
  ...
  const playerTexts = playerMsgs.map((m) => m.text.slice(0, 400));
  return { facts, playerTexts, playerQuote: (playerMsgs.find((m) => m.text.length > 12)?.text || playerMsgs[0]?.text || '').slice(0, 140) };
}
