...
/* The other half: who arrived. A host opens the group, sets the photo and
   posts a welcome, and the player is invited separately, so a group can sit
   there for weeks with nobody in it but us. The creation marks a history we
   can see the whole of; the arrival names who came. Same shape as the
   departures above: the action says somebody joined, the text says who
   ("Carter | Thrill VIP added you to this channel", "sKo added TWIXA",
   "lopper joined the group"). */
export const CREATE_ACTIONS = new Set(['chatCreate', 'channelCreate']);
export const JOIN_ACTIONS = new Set(['chatAddUser', 'chatAddUsers', 'chatJoinedByLink', 'chatJoinedByRequest']);
const ADDED_BY = /^(?<actor>.+?)\s+added\s+(?<target>.+?)\s*$/i;
const JOINED_SELF = /^(?<target>.+?)\s+joined the (?:group|chat|channel)\b/i;

export function joinTarget(text) {
  const t = String(text || '').trim();
  if (!t) return null;
  const m = ADDED_BY.exec(t) || JOINED_SELF.exec(t);
  const target = m?.groups?.target?.trim();
  if (!target) return null;
  // "added you to this channel": the reader account, never the player.
  return /^you\b/i.test(target) ? null : target.replace(/\s+to this (?:group|chat|channel)\s*$/i, '').trim() || null;
}
...
export function extractFacts(messages, { player = null, now = Date.now() } = {}) {
  ...
  // Seeing the group being created means this is the whole history, which is
  // what makes "the player has never been here" provable rather than a guess
  // about messages we cannot see.
  const created = ordered.find((m) => CREATE_ACTIONS.has(m.actionType));
  const playerJoin = ordered.filter((m) => JOIN_ACTIONS.has(m.actionType))
    .find((m) => departureIsPlayer(joinTarget(m.text), { player, staffNames }));
  ...
  const facts = {
    ...
    createdAt: created?.date || null,
    playerJoinedAt: playerJoin?.date || null,
