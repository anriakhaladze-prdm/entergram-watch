...
import { accountStatusLabel, tierLabel } from './playerProfile.js';
...
export function dueAlerts(row, records = {}, { now = Date.now(), urgent = true, quietDays = 7, policy = alertPolicy() } = {}) {
  const due = [];
  // Nobody to chase in a group the player left or never arrived in.
  if (row.flags.left || row.flags.not_joined) return due;
  ...
  if (row.flags.no_contact && row.noContactDays <= quietDays + policy.maxLateDays) {
...
export function formatAlert(kind, row, { env = process.env } = {}) {
  ...
  const lines = [];
  const status = row.accountStatus ? accountStatusLabel(row.accountStatus).toUpperCase() : 'UNKNOWN';
  lines.push(`${k.dot} *${k.label}* · ${who}${row.tier ? ` · ${prettyTier(row.tier)}` : ''} · ${status}`);
  const bits = [];
  if (kind === 'no_contact') {
    bits.push(row.lastStaffAt ? `we last spoke ${humanAge(row.staffQuietDays)} ago${row.lastStaffBy ? ` (${row.lastStaffBy})` : ''}` : 'nothing from us on record');
...
export const prettyTier = tierLabel;
...
export function formatSeedSummary(summary, { env = process.env } = {}) {
  const base = dashboardUrl(env);
  return [
    `*Entergram watch restarted.* Watching ${summary.hosted} player chats.`,
    `${summary.noContact7d} past seven days without contact · ${summary.waiting} waiting on a reply${base ? ` · ${link(`${base}/queue`, 'Open the queue')}` : ''}`,
  ].join('\n');
}
