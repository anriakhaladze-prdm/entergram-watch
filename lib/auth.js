...
//      so only the suffix is trusted. The same callback refuses blocked
//      addresses, static and dashboard-managed.
...
import { ALLOWED_DOMAIN, onDomain, isMonitor, clientIp, describeUa } from './access.js';
import { sessionRevoked, emailBlocked, noteActivity } from './gate.js';
  ...
  return {
    ...
    callbacks: {
      async signIn({ profile }) {
        ...
        if (!onDomain(email)) return '/auth/denied?reason=domain';
        if (await emailBlocked(email)) {
          if (kvConfigured()) await quiet(logSignIn({ email, name: profile?.name || '', at: Date.now(), denied: true, reason: 'blocked', ip: ctx.ip || '', device: ctx.device || '' }));
          return '/auth/denied?reason=blocked';
        }
        return true;
      },
      ...
      async session({ session, token }) {
        ...
        if (!email || !onDomain(email)) return null;
