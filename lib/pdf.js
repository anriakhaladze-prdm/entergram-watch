...
function registerFonts() {
  ...
  Font.register({
    family: 'Oxanium',
    fonts: [500, 600, 700, 800].map((w) => ({ src: path.join(dir, `Oxanium-${w}.ttf`), fontWeight: w })),
  });
...
export const pdfFilename = (row) => `${String(row.playerUsername || row.player || row.chatId).replace(/[^\w.-]+/g, '_').slice(0, 60) || 'player'}.pdf`;
