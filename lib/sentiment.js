...
// The flags in plain words, for the row and the drawer when no model reason
// exists. Ordered by how much each one matters.
const SAYS = {
  leaving: 'talking about leaving',
  scam: 'accuses the site of cheating',
  withdrawal: 'withdrawal not received',
  money_taken: 'says the site is taking all their money',
  bonus: 'bonus not received',
  ignored: 'feels ignored',
  frustration: 'frustrated',
  losing: 'keeps losing',
  competitor: 'mentions a rival',
};
export function describeFlags(flags = []) {
  const parts = Object.keys(SAYS).filter((k) => flags.includes(k)).slice(0, 3).map((k) => SAYS[k]);
...
export function ruleScan(texts = []) {
  ...
  const negative = label === 'at_risk' || label === 'negative';
  return { label, score: Math.round(score * 10) / 10, flags: [...flags], reason: negative ? describeFlags([...flags]) : null, source: 'rules' };
}
...
export function mergeSentiment(rules, model) {
  ...
  return { label: model.label, reason: model.reason || rules.reason || null, evidence: model.evidence || null, flags: rules.flags, ruleScore: rules.score, source: model.source };
}
